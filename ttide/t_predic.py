...
#use kargs to set values other then the defaults
    ...
    for j1 in np.arange(0, ntim, nsub):
        ...
        yout[j1:j2]=np.sum(np.multiply(np.exp(np.outer(24*1j*2*np.pi*freq,tim[j1:j2])),tap),axis=0)+np.sum(np.multiply(np.exp(np.outer(24*-1j*2*np.pi*freq,tim[j1:j2])),tam),axis=0)      
